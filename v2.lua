...
function library:CreateWindow(text)
    ...
    WindowDark.Parent = game.CoreGui
    ...
    local function CQYOZZ_fake_script() -- Main.Dragify 
    ...
    coroutine.wrap(CQYOZZ_fake_script)()
    ...
    coroutine.wrap(AECJVRV_fake_script)()
    


local gamerlib = {}

function gamerlib:CreateTab(name, visible)
        ...
        tabtext.Text = name or "NewTab"
    end
function gamerlib:CreateButton(name,tab,callback)
    ...
    buttonlabel.Text = name or "NewButton"
end

function gamerlib:CreateToggle(name, tab, callback)
    local switchactions = {}
    enabled = false
    ...
    toggleholder.Name = "toggleholder" or name
    toggleholder.Parent = tabs[tab]
    ...
    function Trigger()
        enabled = not enabled
        pointer:TweenPosition(enabled and UDim2.new(0.5,0,0,0) or UDim2.new(-0.018, 0,0, 0), "In", "Sine", 0.2)
...
function gamerlib:CreateSlider(name, tab, minvalue, maxvalue, callback)
    ...
    sliderbutton.Position = UDim2.new(0, 0, 0.595238149, 0)
    sliderbutton.Size = UDim2.new(0, 105, 0, 9)
    ...
    slidernumber.Position = UDim2.new(0.847619057, 0, 0, 0)
    slidernumber.Size = UDim2.new(0, 16, 0, 21)
    ...
    sliderbutton.MouseButton1Down:Connect(function()
        down = true
        Value = math.floor((((tonumber(maxvalue) - tonumber(minvalue)) / 105) * slidermain.AbsoluteSize.X) +tonumber(minvalue)) or 0
        slidernumber.Text = Value
        pcall(callback, Value)
        slidermain:TweenSize(UDim2.new(0, math.clamp(mouse.X - slidermain.AbsolutePosition.X, 0, 105), 0, 9), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, .07)
    ...
    uis.InputEnded:connect(function(key)
        if key.UserInputType == Enum.UserInputType.MouseButton1 and down then
            ...
            slidernumber.Text = Value
            pcall(callback, Value)
            slidermain:TweenSize(UDim2.new(0, math.clamp(mouse.X - slidermain.AbsolutePosition.X, 0, 105), 0, 9), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0.1)
        end        
    end)
    local ss = {}
